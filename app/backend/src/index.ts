...
(
  async () => {
    try {
      const portNumberAsString = getStringEnvVar("EXPRESS_SERVER_PORT");

      if(!portNumberAsString) {
        throw new Error(`Missing environment variable for express server port`);
      }

      const port: number = parseInt(portNumberAsString, 10);
      if(isNaN(port)) {
        throw new Error(`Invalid number value of environment variable for express server port`);
      }
