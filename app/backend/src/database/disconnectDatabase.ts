...
const disconnectDatabase = async (): Promise<void> => {
  ...
  } catch (databaseDisconnectionError: unknown) {
    if(databaseDisconnectionError instanceof Error) {
      console.error("Error occurred while closing database connection pool. Error details: ", databaseDisconnectionError.message);
    } else {
      console.error("Unknown error occurred while closing database connection pool. Error details: ", databaseDisconnectionError);
    }
  }
};
