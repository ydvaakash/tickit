...
import pgdbpool from "./databasePool";
import { PoolClient } from 'pg';

const connectDatabase = async (): Promise<void> => {
  let pgdbPoolClient: PoolClient | undefined;

  try {
    pgdbPoolClient = await pgdbpool.connect();
    console.log('Database connected successfully.');
    pgdbPoolClient.release();
  } catch (databaseConnectionError: unknown) {
    if(databaseConnectionError instanceof Error) {
      pgdbPoolClient?.release();
      throw new Error(`Error connecting with database. Error details: ${databaseConnectionError.message}`);
    } else {
      pgdbPoolClient?.release();
      throw new Error(`Unknown error occurred while trying to connect with database. Error details: ${databaseConnectionError}`);
    }
  }
};
