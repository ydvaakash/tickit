...
const dbuser: string = getStringEnvVar("PGDBUSERNAME");
const dbUserPassword: string = getStringEnvVar("PGDBUSERPASSWORD");
const dbHost: string = getStringEnvVar("PGDBHOST");
const dbHostPort: string = getStringEnvVar("PGDBHOSTPORT");
const dbName: string = getStringEnvVar("PGDBNAME");

if(!dbuser) {
  throw new Error('Missing value of database user in environment variables for database connection.');
} else if(!dbUserPassword) {
  throw new Error('Missing value of database user password in environment variables for database connection.');
} else if(!dbHost) {
  throw new Error('Missing value of database host in environment variables for database connection.');
} else if(!dbHostPort) {
  throw new Error('Missing value of database host port in environment variables for database connection.');
} else if(!dbName) {
  throw new Error('Missing value of database name in environment variables for database connection.');
}

const dbHostPortAsNumber: number = parseInt(dbHostPort, 10);

if(isNaN(dbHostPortAsNumber)) {
  throw new Error(`Invalid number value of environment variable for database host port`);
}
...
const dbconfig: PoolConfig = {
  ...
  host: dbHost,
  port: dbHostPortAsNumber,
  database: dbName
};

const pgdbpool: Pool = new Pool(dbconfig);

// Event listener for connection errors
pgdbpool.on("error", async (dbConnectionError) => {
  await pgdbpool.end();
});
