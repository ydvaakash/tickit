...
// Middleware to validate if the 'req' object has valid and non-empty HEADERS

import { NextFunction, Request, Response } from "express";

const validateExistanceOfRequestHeaders = (req: Request, res: Response, next: NextFunction): void => {
  if(!req.headers || typeof req.headers !== 'object' || Object.keys(req.headers).length == 0) {
    ...
    console.log("validateExistanceOfRequestHeaders middleware failed.");
    return ;
  }

  console.log("validateExistanceOfRequestHeaders middleware passed.");
  next();
};

export { validateExistanceOfRequestHeaders };