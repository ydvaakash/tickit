...
import { z } from "zod";
  ...
  .regex(/^[A-Za-z][A-Za-z\s'-]*$/, {
    message: "First name must start with a letter and only contain letters, spaces, apostrophes, or hyphens."
  })
  
  // Disallow repeating the same character 3 or more times
  .refine(val => !/(.)\1{2,}/.test(val), {
    message: "First name cannot have the same character repeated more than twice in a row."
  })
  
  // Disallow repeated non-letter characters (like --, '', or multiple spaces)
  .refine(val => !/[\s'-]{2,}/.test(val), {
    message: "First name cannot contain repeating non-letter characters like --, '', or multiple spaces."
  })
...
export type firstNameTypeFromZod = z.infer<typeof firstNameZodSchema>;