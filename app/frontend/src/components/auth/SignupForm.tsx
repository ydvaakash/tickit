...
function SignupForm() {
  ...
  const setServerError = useSetRecoilState(serverErrorAtom);

  const submitSignupForm = async (formData: signupFormFieldsType) => {
    // console.log('Signup Form SUbmitted');

    try {
      const resultFromBackend = await newUserSignupApi(
        formData.first_name,
        formData.last_name,
        formData.email,
      ...
      // using Recoil state management library, create a state variabble as 'serverError'
      // basis the error message received from the backend in the response here, update the value of state variable
      // display this error message from the state variable in the UI to the user.

      // if(err.response) {
      //   const statusCode = err.response.status;
      //   const backendErrorMessage = err.response.data?.message | "Unknown error occurred";
      ...
      if(axios.isAxiosError(err)) {
        if(err.response) {
          ...
          if(errorStatusCode === 403) {
            ...
            setServerError("Missing credentials!");
            ...
            setServerError("Oops! Something went wrong on server. Please try again later.");
          } else if(errorStatusCode === 500) {
            setServerError("Oops! Something went wrong on server. Please try again later.");
          } else if(errorStatusCode === 508) {
            setServerError("Signup successful!");
          }
        } else if(err.request) {
          // The request was made but no response was received
          setServerError("Oops! Something unexpected happened. Please try again.");
        } else {
          // Something happened in setting up the request that triggered an Error
          setServerError("Couldn't send request to server. Please try again.");
        }
      } else {
  ...
  return <>
    ...
    <input {...register("last_name", {required: false})} placeholder='Last Name' />
    <input {...register("email", {required: true})} placeholder='abc@xyz.com' />
    <input {...register("user_password", {required: true})} placeholder='Password' /> */}


    <form id='signupform' onSubmit={handleSubmit(submitSignupForm)}>
      <input type='text' id='firstNameInputBox' {...register("first_name")} autoComplete='on' autoFocus placeholder='First name'></input>
      ...
      <input type='text' id='lastNameInputBox' {...register("last_name")} autoComplete='on' placeholder='Last name'></input>
      {/* {errors.last_name && errors.last_name.message} */}
      {errors.last_name && enduserFriendlySignupZodErrorMessages('last_name', errors.last_name.message)}
      <input type='email' id='emailInputBox' {...register("email")} autoComplete='on' placeholder='Ex. abc@xyz.com'></input>
      {/* {errors.email && errors.email.message} */}
      {errors.email && enduserFriendlySignupZodErrorMessages('email', errors.email.message)}
      <input type='password' id='userPasswordInputBox' {...register("user_password")} autoComplete='on' placeholder='Password'></input>
      ...
      <input type='submit' id='signupFormSubmitButton' value="Sign-up"></input>
    </form>
